 ...
 */
void get_stats(char *buf, long long unsigned int *run_ticks, long long unsigned int *wait_ticks,
    long long unsigned int *nran)
{
    char *ptr;
...
int main(int argc, char *argv[])
{
    ...
    char id[32];
    long long unsigned int run_ticks, wait_ticks, nran;
    long long unsigned int orun_ticks=0, owait_ticks=0, oran=0;
    ...
    while (fp = fopen(statname, "r")) {
	    ...
	    if (verbose)
		printf("%s %llu(%llu) %llu(%llu) %llu(%llu) %4.2f %4.2f\n",
		...
		printf("%s avgrun=%4.2fus avgwait=%4.2fus\n",
		    id, nran - oran ?
			(double)(run_ticks-orun_ticks)/(nran - oran)/1000 : 0,
		    nran - oran ?
			(double)(wait_ticks-owait_ticks)/(nran - oran)/1000 : 0);
	    fclose(fp);
	    oran = nran;
