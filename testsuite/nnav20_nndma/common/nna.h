...
#define NNAPC_INSN(insn) {                                              \
  if (m_nna_pc_insn0_sel == NNAPC_INSN_##insn) m_nna_insn0_num++;       \
  if (m_nna_pc_insn1_sel == NNAPC_INSN_##insn) m_nna_insn1_num++;       \
  if (m_nna_pc_insn2_sel == NNAPC_INSN_##insn) m_nna_insn2_num++;}
#define NNAPC_COUNT(type,n) {                                           \
  if (m_nna_pc_cnt0_sel == NNAPC_CNT_##type) m_nna_count0+=n;           \
  if (m_nna_pc_cnt1_sel == NNAPC_CNT_##type) m_nna_count1+=n;           \
  if (m_nna_pc_cnt2_sel == NNAPC_CNT_##type) m_nna_count2+=n;}
...
#define WFCW_LP_SETUP(g,ptr) {						\
    ...
    if (m_nna_wfcw_lpend[g]==1<<NNA_WFCW_ABIT) m_nna_wfcw_lpend[g] = 0;	\
