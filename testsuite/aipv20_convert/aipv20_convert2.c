...
int aipv20_convert()
{
	...
	for (i = 0; i < box_num; i++) {
		tnode[i].src_ybase 		= src_pbase_y + offset_y;
		tnode[i].src_cbase 		= src_pbase_uv + offset_uv;
		tnode[i].dst_base  		= (uint32_t)dst_bo.paddr;
		tnode[i].src_h     		= box_h;
		tnode[i].src_w     		= box_w;
		tnode[i].src_stride     = src_stride;
		tnode[i].dst_stride     = dst_stride;
		tnode[i].task_len       = task_h;
		ret = iaic_convert_config(&iaic_ctx,
				AIP_DAT_POS_DDR,
				AIP_DAT_POS_DDR,
				AIP_ORDER_BGRA,
				255, 16, 128,
		...
		for (j = 0; j < box_h; j += task_h) {
			...
			iaic_convert_start(&iaic_ctx, task_dst_paddr, task_h);
			iaic_convert_wait(&iaic_ctx);
		}
		//iaic_convert_reg_read(&iaic_ctx);
		ret = iaic_convert_exit(&iaic_ctx);
