...
#define BTRAM_BSIZE (512 / 8)
#define BTRAM_ENTRY (256)
#define BTRAM_SIZE (BTRAM_ENTRY * BTRAM_BSIZE)
...
#define NNARA_WR_RADDR_ADD 0x14
...
// DATA
#define NNARA_RDD_REG 0
#define NNARA_RDD_WFCW 1
// F-RAM address bit, io=64, 32kB
#define NNARA_FR_ABIT 12
// BT-RAM addr bit, io=512, 16kB
#define NNARA_BTR_ABIT 8
// WFCW has 2 8-word
#define NNARA_WFCW_ABIT 3
// F-RAM write align 4-banks
#define NNARA_FR_WALIGN_BITS 2
// WRAM
#define NNARA_WR_WADDR 0x1c
// BTRAM
#define NNARA_BTR_WADDR 0x1e
#define NNARA_BTR_RADDR 0x02
// DRead
#define NNARA_MSF_MODE 0x06
#define NNARA_Q_BIT 0x04
#define NNARA_QD_CONFIG 0x05
#define NNARA_RDND_NUM 0x07
#define NNARA_OC_LSH 0x03
#define NNARA_PIXEL_OUT0 0x08
#define NNARA_PIXEL_OUT1 0x09
/*
#define NNARA_PIXEL_OUT2    0x0a
...
#define NNARA_PIXEL_OUT7    0x0f
*/
// FRAM
#define NNARA_FR_WADDR0 0x10
#define NNARA_FR_WADDR1 0x11
#define NNARA_FR_WADDR0_ADD 0x12
#define NNARA_FR_WADDR1_ADD 0x13
#define NNARA_FR_CONFIG 0x14
#define NNARA_F_BIT 0x15
#define NNARA_WFCW_WPTR 0x18
#define NNARA_WFCW_INP 0x19
#define NNARA_WFCW_END 0x1a
#define NNARA_WFCW_PN 0x1b
...
#define NNARA_WFCW_FMTERR 5
#define NNARA_RDQR_ERROR 6
#define NNARA_QRFIFO_ERROR 7
...
#define NNAPCG0_BLOCK_DRQR 2
#define NNAPCG0_BLOCK_QOCB 3
#define NNAPCG0_WAIT_CPOC 4
#define NNAPCG0_WAITFF_QOCB 5
#define NNAPCG0_MAC_WAIT_RCFL 6
#define NNAPCG0_DWRF_WAIT_RCFL 7
#define NNAPCG0_DWRW_WAIT_RCFL 8
#define NNAPCG0_MACOP_FREX_NUM 9
#define NNAPCG0_WAIT_LSRS 10
#define NNAPCG0_WAIT_RWRM 11
...
#define NNACMD_DATAUC_TYPE 1
#define NNACMD_D_TYPE_BIT 5
...
#define NNACMD_RDBT 4
#define NNACMD_QOCB 8
...
/*** for NNA's functions ***/
/***************************/
#define NNAPC_INSN(insn)                                                                           \
    {                                                                                              \
        if (m_nna_pc_insn0_sel == NNAPC_INSN_##insn)                                               \
            m_nna_insn0_num++;                                                                     \
        if (m_nna_pc_insn1_sel == NNAPC_INSN_##insn)                                               \
            m_nna_insn1_num++;                                                                     \
        if (m_nna_pc_insn2_sel == NNAPC_INSN_##insn)                                               \
            m_nna_insn2_num++;                                                                     \
    }
#define NNAPC_COUNT(type, n)                                                                       \
    {                                                                                              \
        if (m_nna_pc_cnt0_sel == NNAPC_CNT_##type)                                                 \
            m_nna_count0 += n;                                                                     \
        if (m_nna_pc_cnt1_sel == NNAPC_CNT_##type)                                                 \
            m_nna_count1 += n;                                                                     \
        if (m_nna_pc_cnt2_sel == NNAPC_CNT_##type)                                                 \
            m_nna_count2 += n;                                                                     \
    }

// push to qrfifo
#define PUSH_QRFIFO(qr)                                                                            \
    if (m_nna_qrfifo_level < NNA_QRFIFO_ENTRY)                                                     \
        m_nna_qrfifo[m_nna_qrfifo_level++] = qr;                                                   \
    else if (m_nna_qrfifo_ex_level < NNA_QRFIFOEX_ENTRY)                                           \
        m_nna_qrfifo_ex[m_nna_qrfifo_ex_level++] = qr;                                             \
    else {                                                                                         \
        m_nna_ewsr |= 1 << NNARA_QRFIFO_ERROR;                                                     \
        if (m_nna_inte & (1 << NNARA_QRFIFO_ERROR)) {                                              \
            qr_fifo_full = 1;                                                                      \
            nna_fault_interrupt_ctl(1);                                                            \
        }                                                                                          \
    }

// wfcw loop setup
#define WFCW_LP_SETUP(g, ptr)                                                                      \
    {                                                                                              \
        m_nna_wfcw_lpstt[g] = (ptr + 1) & ((1 << NNA_WFCW_ABIT) - 1);                              \
        m_nna_wfcw_lpend[g] = m_nna_wfcw[g][ptr].lpe;                                              \
        if (m_nna_wfcw_lpend[g] == 1 << NNA_WFCW_ABIT)                                             \
            m_nna_wfcw_lpend[g] = 0;                                                               \
        m_nna_wfcw_lpn_cur[g] = m_nna_wfcw[g][ptr].lpn - 1;                                        \
        m_nna_wfcw_rptr[g] = (ptr + 1) & ((1 << NNA_WFCW_ABIT) - 1);                               \
        error += m_nna_wfcw_lpstt[g] == m_nna_wfcw_lpend[g];                                       \
        /*error += m_nna_wfcw[g][ptr].lpn == 0; */                                                 \
        error += m_nna_wfcw_lpend[g] >= 1 << NNA_WFCW_ABIT;                                        \
        /*error += m_nna_wfcw_lpend[g] > m_nna_wfcw_end[g] && m_nna_wfcw_end[g] != 0; */           \
    }
#endif /* __NNA_H__ */
