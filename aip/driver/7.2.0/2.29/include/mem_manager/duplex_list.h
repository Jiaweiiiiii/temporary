...
#define DUPLEX_LIST_HEAD(name) struct list_head name = LIST_HEAD_INIT(name)
 ...
 */
static inline void __duplex_list_add(struct list_head *newnode, struct list_head *prev,
                                     struct list_head *next) {
    next->prev = newnode;
    newnode->next = next;
    newnode->prev = prev;
...
static inline void duplex_list_add_tail(struct list_head *newnode, struct list_head *head) {
    __duplex_list_add(newnode, head->prev, head);
}
 ...
 */
static inline void duplex_list_replace(struct list_head *old, struct list_head *newnode) {
...
static inline void duplex_list_replace_init(struct list_head *old, struct list_head *newnode) {
    duplex_list_replace(old, newnode);
    INIT_DUPLEX_LIST_HEAD(old);
}

static inline void duplex_list_prev_insert(struct list_head *list, struct list_head *newnode) {
...
static inline void duplex_list_next_insert(struct list_head *list, struct list_head *newnode) {
 ...
 */
static inline void duplex_list_move_tail(struct list_head *list, struct list_head *head) {
    __duplex_list_del_entry(list);
 ...
 */
static inline int duplex_list_is_last(const struct list_head *list, const struct list_head *head) {
 ...
 */
static inline int duplex_list_empty(const struct list_head *head) { return head->next == head; }
 ...
 */
#define duplex_list_entry(ptr, type, member) ((type *)(((char *)(ptr)) - offsetof(type, member)))
 ...
 */
#define duplex_list_first_entry(ptr, type, member) duplex_list_entry((ptr)->next, type, member)
 ...
 * Note, that list is expected to be not empty.
 */
#define duplex_list_last_entry(ptr, type, member) duplex_list_entry((ptr)->prev, type, member)
 ...
 */
#define duplex_list_for_each(pos, head) for (pos = (head)->next; pos != (head); pos = pos->next)
 ...
 */
#define __duplex_list_for_each(pos, head) for (pos = (head)->next; pos != (head); pos = pos->next)
...
#define duplex_list_for_each_entry(pos, head, member)                                              \
    for (pos = duplex_list_entry((head)->next, typeof(*pos), member); &pos->member != (head);      \
