...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < box_num; i++) {
        infos[i].box.x = 0;
        infos[i].box.y = 0;
        infos[i].box.w = cfg->src_w;
        infos[i].box.h = cfg->src_h;
        infos[i].wrap = 0;
    ...
    bs_affine_start(&src, box_num, &dut, infos, coef, offset);
    bs_affine_wait();
    bs_affine_mdl(&src, box_num, &gld, infos, coef, offset);
