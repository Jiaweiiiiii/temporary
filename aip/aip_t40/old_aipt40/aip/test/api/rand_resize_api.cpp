...
#endif
    if (cfg.src_locate) {
        cfg.src_base = (uint8_t*)bscaler_malloc_oram(256, src_buf_size);
    } else {
        cfg.src_base = (uint8_t*)bscaler_malloc(256, src_buf_size);
    }

    if (cfg.dst_locate) {
        cfg.dst_base = (uint8_t*)bscaler_malloc_oram(64, dst_buf_size);
    } else {
        cfg.dst_base = (uint8_t*)bscaler_malloc(64, dst_buf_size);
    }
