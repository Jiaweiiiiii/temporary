...
int main(int argc, char** argv)
{
    ...
    if (argc < 2) {
        fprintf(stderr, "%s [image_path]/entertainers_2048x1080.nv12\n", argv[1]);
        exit(1);
    }
    ...
    int dst_size = dst_w * dst_h * box_num * 4;//3 / 2;
    ...
    for (int i = 0; i < box_num; i++) {
        dst[i].base = dst_base + i * dst_w * dst_h *4; //  * 3 / 2;
        dst[i].base1 = NULL;//fixme
        dst[i].format = BS_DATA_BGRA;// BS_DATA_NV12;
        dst[i].chn = 4;//1;
        dst[i].height = dst_h;
        dst[i].width = dst_w;
    ...
    char str[128];
    sprintf(str,"resize_nv2rgb_%d_%d_%.1fms.jpg",img_w,dst_w,time_use);
    stbi_write_jpg(str, dst_w, dst_h * box_num, 4, dst_base, 90);

/* FILE *fpo;
    fpo = fopen("resize_out_256_512.", "wb+");
    if (fpo == NULL) {
