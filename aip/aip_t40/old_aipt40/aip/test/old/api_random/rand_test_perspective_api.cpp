...
int main(int argc, char** argv)
{
    ...
    for (int idx = idx_st; idx < idx_end; idx++) {
        ...
        std::string src_name = src_base + std::to_string((long long)idx) + ".png";
        std::string dut_base = "dut";
        std::string dut_name = dut_base + std::to_string((long long)idx) + ".png";
        std::string gld_base = "gld";
        std::string gld_name = gld_base + std::to_string((long long)idx) + ".png";
#endif
        ...
        //debug_point(105, 0);
#endif
        // golden int perspective
        perspective_whole_int(&src, box_num, &gld, &info, coef, offset);

        // dut integer perspective
#if (defined CSE_SIM_ENV) || (defined CHIP_SIM_ENV) || (defined EYER_SIM_ENV)
        bs_perspective_start(&src, box_num, &dut, &info, coef, offset);
        bs_perspective_wait();
        bs_perspective_mdl(&src, box_num, &gld, &info, coef, offset);
#else
        bs_perspective_mdl(&src, box_num, &dut, &info, coef, offset);
#endif
        ...
        stbi_write_jpg("out_src.jpg", src_w, src_h * box_num, 4, src.base, 100);
        stbi_write_jpg("out_gld.jpg", dst_w, dst_h * box_num, 4, gld.base, 100);
        stbi_write_jpg("out_dut.jpg", dst_w, dst_h * box_num, 4, dut.base, 100);
#endif
        //check
#if 1
