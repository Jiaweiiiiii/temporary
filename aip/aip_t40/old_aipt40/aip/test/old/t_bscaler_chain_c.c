...
void bscaler_frmc_init(bs_hw_once_cfg_s *cfg, frmc_random_s *radom_cfg, uint8_t mode_sel)
{
...
int TEST_NAME()
{
    ...
    do{
        ...
        /*bs_hw_once_cfg_s  frmc_cfg_inst[frame];
        bs_hw_once_cfg_s  frmc_mdl_inst[frame];

        frmc_random_s  radom_cfg_inst[frame];*/
        bs_hw_once_cfg_s *frmc_cfg_inst = (bs_hw_once_cfg_s*)malloc(frame * (sizeof(bs_hw_once_cfg_s)));
        bs_hw_once_cfg_s *frmc_mdl_inst = (bs_hw_once_cfg_s*)malloc(frame * (sizeof(bs_hw_once_cfg_s)));
        ...
        for(n=0; n<frame; n++){
            ...
            bs_hw_once_cfg_s *frmc_cfg = &(frmc_cfg_inst[n]);
        ...
        uint32_t  frmc_chain_irq_mask =1;
        //start chain
        uint32_t chain_bus = 0;
        bscaler_write_reg(BSCALER_FRMC_CHAIN_CTRL,frmc_chain_irq_mask<<3 |
                          chain_bus<<1 |
                          1<<0);//start hw
        printf("----hw start----\n");
        ...
        //polling 1
        printf("----polling----\n");
        while (((bscaler_read_reg(BSCALER_FRMC_CHAIN_CTRL, 0) & 0x1) != 0) | ((bscaler_read_reg(BSCALER_FRMC_CTRL, 0) & 0x1F) != 0)){
        }
        //for mdl init
        for(n=0; n<frame; n++){
            bs_hw_once_cfg_s *frmc_cfg = &frmc_cfg_inst[n];
            bs_hw_once_cfg_s *frmc_mdl = &frmc_mdl_inst[n];
            frmc_random_s  *radom_cfg = &radom_cfg_inst[n];
