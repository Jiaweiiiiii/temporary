...
int TEST_NAME()
{
    ...
    //comp default
    printf("Step1---\n");
    printf("Step1: check the default value\n");
    printf("Step1---\n");
    for(i=0;i<58;i++){
        check_bypass = (i== 6);
        if(check_bypass){
            printf("NOTE:default check bypass %d\n", i);
        }else{
            if( i == 6 ){
            ...
            for(k=0; k<size;k++){
                read_default = bscaler_read_reg(bscaler_reg_addr[i],0);
                if(read_default==bscaler_reg_default[i]){
                    //printf("success:%d(%d); 0x%x\n",i,k,read_default);
                }else{
                    printf("error:%d(%d); GLD:0x%x, DUT:0x%x\n",i,k,bscaler_reg_default[i],read_default);
#ifdef EYER_SIM_ENV
                    eyer_stop();
#endif
                    return 1;
    ...
    uint32_t read_ff;
    printf("Step2---\n");
    printf("Step2: check the ff_value\n");
    printf("Step2---\n");
    ...
    for(i=0;i<58;i++){
        check_bypass = (i==0 | i==13 | i==27 |
                        i==28 | i==39 | i==53);
        if(check_bypass){
            printf("NOTE:ff_value check bypass CTRL reg %d\n", i);
        }else{
            if( i == 6 ){
                size = 64;//fix me init lut_ram
            }
            ...
            for(k=0; k<size;k++){
                bscaler_write_reg(bscaler_reg_addr[i],0xffffffff);
                read_ff = bscaler_read_reg(bscaler_reg_addr[i],0);
    ...
    uint32_t read_5a;
    printf("Step3---\n");
    printf("Step3: check the 5a_value\n");
    printf("Step3---\n");
    ...
    for(i=0;i<58;i++){
        check_bypass = (i==0 | i==13 | i==27 |
                        i==28 | i==39);
        if(check_bypass){
            printf("NOTE:5a_value check bypass CTRL reg %d\n", i);
        }else{
            if( i == 6 ){
                size = 64;//fix me init lut_ram
            }
            ...
            for(k=0; k<size;k++){
                bscaler_write_reg(bscaler_reg_addr[i],0x5a5a5a5a);
                read_5a = bscaler_read_reg(bscaler_reg_addr[i],0);
    ...
    uint32_t read_a5;
    printf("Step4---\n");
    printf("Step4: check the a5_value\n");
    printf("Step4---\n");
    ...
    for(i=0;i<58;i++){
        check_bypass = (i==0 | i==13 | i==27 |
                        i==28 | i==39  | i==53);
        if(check_bypass){
            printf("NOTE:a5_value check bypass CTRL reg %d\n", i);
        }else{
            if( i == 6 ){
                size = 64;//fix me init lut_ram
            }
            ...
            for(k=0; k<size;k++){
                bscaler_write_reg(bscaler_reg_addr[i],0xa5a5a5a5);
                read_a5 = bscaler_read_reg(bscaler_reg_addr[i],0);
