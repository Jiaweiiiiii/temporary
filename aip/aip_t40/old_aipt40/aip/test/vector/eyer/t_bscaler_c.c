...
void bscaler_frmc_init(bsc_hw_once_cfg_s *cfg, frmc_random_s *radom_cfg)
{
    ...
    //src info
    cfg->src_box_h = rand()%1024+1;
    if(bpp_mode == 0){
        cfg->src_box_w = rand()%2047+1;//1->2048
    }else if(bpp_mode == 1){
        cfg->src_box_w=rand()%1024+1;//1->>1024
    }else if(bpp_mode == 2){
        cfg->src_box_w=rand()%512+1;//1->>512
    }else{
        cfg->src_box_w=rand()%256+1;//1->>256
    }
    ...
    if(cfg->affine==0 ){
        if(cfg->box_mode==1){
            cfg->dst_box_w = rand()%64+1;//bresize:1-64
...
int TEST_NAME()
{
    ...
    uint32_t seed_idx = 0;
    uint32_t seed_num = 50;//000;

    uint32_t seed_time;
    uint32_t seed;
    seed_time = (unsigned)time(NULL);
    //seed_time = 1595314552;
    do {
        uint32_t case_no_check=0;
        uint32_t case_time_out=0;
        ...
        bsc_hw_once_cfg_s  frmc_cfg_inst;
        bsc_hw_once_cfg_s  *frmc_cfg = &frmc_cfg_inst;

        bsc_hw_once_cfg_s  frmc_mdl_inst;
        bsc_hw_once_cfg_s  *frmc_mdl = &frmc_mdl_inst;
