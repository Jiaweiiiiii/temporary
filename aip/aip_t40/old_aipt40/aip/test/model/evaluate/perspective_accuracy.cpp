 ...
 *             ALL RIGHTS RESERVED
 *
 * File       : perspective_accuracy.cpp
 * Authors    : jmqi@taurus
 * Create Time: 2020-05-20:20:21:23
 * Description: test perspective used float and int.
 *
 */
...
int main(int argc, char** argv)
{
    ...
    if (argc < 2) {
        ...
        fprintf(stderr, "%s ../image/perspective_matrix_from_real_picture_64x64-0521.data 0 1 ../image/chess.bmp\n", argv[0]);
        exit(1);
    }
    ...
    for (int idx = idx_st; idx < idx_end; idx++) {
        ...
        std::string src_name = src_base + std::to_string(idx) + ".png";
        std::string dut_base = "dut";
        std::string dut_name = dut_base + std::to_string(idx) + ".png";
        std::string gld_base = "gld";
        std::string gld_name = gld_base + std::to_string(idx) + ".png";
#endif
        ...
        data_info_s src = {NULL, NULL, BS_DATA_ARGB, 4, src_w, src_h, src_line_stride};
        data_info_s gld = {NULL, NULL, BS_DATA_ARGB, 4, dst_w, dst_h, dst_line_stride};
        data_info_s dut = {NULL, NULL, BS_DATA_ARGB, 4, dst_w, dst_h, dst_line_stride};
...
//        debug_point(132, 13);
        ...
        // golden float perspective
        perspective_whole_float(&src, box_num, &gld, &info, coef, offset);

        // golden integer perspective
        perspective_whole_int(&src, box_num, &dut, &info, coef, offset);
        ...
        for (int i = 0; i < dst_h; i++) {
            for (int j = 0; j < dst_w; j++) {
                for (int k = 0; k < 3; k++) {
                    ...
                    int pixel_diff = abs(g_val - d_val);
                    ...
                    if ((g_val != d_val) && (abs(g_val - d_val) > 5)) {
                        //printf("(%d,%d,%d) : (G) %d -- (E) %d\n", j, i, k, g_val, d_val);
                    }
                }
            }
        }
        ...
        diffs[idx] = diff;
        printf("max_pixel_diff=%d -- (%d,%d)\n", max_pixel_diff, dx_r, dy_r);
        if (diff > 10.0) {
            printf("####### idx = %d ##############\n", idx);
            printf("diff = %f\n", diff);
        }
