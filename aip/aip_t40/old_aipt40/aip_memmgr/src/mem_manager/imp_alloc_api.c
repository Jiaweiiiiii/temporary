...
static void *IMP_create_units(Alloc *alloc, uint32_t unitsize, uint32_t incunits, const char *name)
{
	unsigned int i = 0;
	...
	unitsize = unitsize > sizeof(struct memery_header) ? unitsize : sizeof(struct memery_header); /*最少申请的大小*/
	memsize = unitsize * incunits;

	manager = (struct unit_manager *)imp_user_alloc(alloc, sizeof(struct unit_manager) + memsize, name);
	if(!manager)
		return NULL;
	...
	//	printf("%d %s list->next = %p, list->prev\n", __LINE__,__func__ ,list->next, list->prev);
		duplex_list_add_tail(list, &(manager->free));
	}
	return (void *)manager;
}

void IMP_destroy_units(Alloc *alloc, void *handle)
{
...
void *IMP_alloc_unit(Alloc *alloc, void *handle)
{
	struct unit_manager *manager = (struct unit_manager *)handle;
	struct list_head *list = NULL;
	struct memery_header *mem = NULL;
	...
	if(duplex_list_empty(&manager->free)){
		unsigned int memsize = manager->unitsize * manager->inc_units + sizeof(struct memery_header);
		unsigned int i = 0;
		mem = (struct memery_header *)imp_user_alloc(alloc, memsize, manager->name);//manager user memory
		if(!mem){
	...
	duplex_list_del(list);//删除节点
	pthread_mutex_unlock(&(manager->mutex));
//	memset(list, 0, manager->unitsize);
	return (void *)list;
}

void IMP_free_unit(void *handle, void *addr)
{
	struct unit_manager *manager = (struct unit_manager *)handle;
...
static int32_t imp_buf_alloc_init(Alloc *alloc)
{
	Bufunit_s *root = NULL;
	Bufunit_s *unit = NULL;
	alloc->buf_handle = IMP_create_units(alloc, sizeof(Bufunit_s), BUF_MANAGER_NODE_CNT, "buffalloc");
...
static Bufunit_s *alloc_bufunit(Alloc *alloc, Bufunit_s *root, int size, uint32_t alignsize)
{
	...
	if(freeunit){
		 ...
		 * other case :
		 * |---------------------|			|---------------------------------------------|
		 * | preunit | free unit | -------->| preunit len + offset | free unit | new unit |
		 * |---------------------|			|---------------------------------------------|
		 * */
...
int32_t Local_HeapDeInit(void *heap)
{
	Alloc *alloc = NULL;
	AllocInfo *info = NULL;
	int ret = IMP_SUCCESS;
...
{
	return Local_alignAlloc(heap, 1, nbytes);
}
...
void Local_Dump_List(void *heap)
{
	...
	duplex_list_for_each_entry(unit, &root->children, brothers){
		...
		if(!duplex_list_empty(&unit->children)){
			duplex_list_for_each_entry(son, &unit->children, brothers){
				if(son->status == ALLOC_BUF_FREE)
					free += son->length;
				size += son->length;
				printf("        %d): state = %s, vaddr = 0x%08x, len = %dBytes\n", son_idx, son->status ? "busy" : "free",
											son->vaddr, son->length);
				son_idx++;
			}
		}else{
		...
		total_free += free;
		printf("   this over: vaddr = 0x%08x, len = %dBytes, free = %dBytes, using rate = %0.2f%%\n", unit->vaddr, unit->length, free, (float)(unit->length - free)/ unit->length * 100);
		idx++;

	}
	printf("all over: vaddr = 0x%08x, len = %dBytes, free = %dByptes, using rate = %0.2f%%\n", root->vaddr, root->length, total_free, (float)(root->length - total_free)/ root->length* 100);
	return;
}
