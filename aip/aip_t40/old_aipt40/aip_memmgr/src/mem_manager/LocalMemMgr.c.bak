...
void Local_Dump_List(void *heap) {
	...
	printf("[%5d]:local_status (%p)=0x%x, local_next(%p)=0x%x, local_prev(%p)=0x%x\n", i++,
	       addr, local_status(addr), addr, local_next(addr), addr, local_prev(addr));
	addr = (void *)local_next(addr);
    }
}
...
static int Local_Dealloc_Inner(void *heap, void *address) {
    uintptr_t block;
    uintptr_t lblock;
    uintptr_t rblock;
    ...
    */
    ...
    if ((lblock != 0) && (local_status(lblock) == FREE) && (rblock != 0) &&
		(local_status(rblock) == FREE)) {
...
int Local_Dealloc(void *heap, void *address) {
    ...
    */
    ...
    return Local_Dealloc_Inner(heap, address);
}
/*end Local_Dealloc*/
...
int Local_ReservedSize(void *heap){
	int total_size = (local_last(heap) - local_first(heap) - SIZE_HEADER);
	uintptr_t addr = (void *)local_first(heap);
	int i=0;
    do{
		int free_size = 0;
		if(local_status(addr)==FREE){
