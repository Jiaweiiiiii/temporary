...
#define _GNU_SOURCE
/*
 * USE_RMEM: ddr memory space comes from rmem, else ddr memory comes from system memory; For from
 * system memory need more time to more larger ddr size, so advice to open USE_RMEM
 */
#define USE_RMEM
#define ORAM_TEST_SIZE (8 * 1024 * 1024)
#define NMEM_TEST_SIZE (12 * 1024 * 1024)
...
int DRIVERS_VERSION = 0x20220525;
...
#ifdef VENUS_DEBUG
	printf("CMD Line Nmem Size:%x, Addr:0x%08x\n", size, addr);
#endif
...
int __aie_mmap(int ddr_mem_size, int b_use_rmem, nna_cache_attr_t desram_cache_attr,
...
#ifdef VENUS_DEBUG
	printf(
	"mmaped __nndma_io_vbase=%p, NNDMA_IO_BASE=0x%x, NNDMA_IO_SIZE=0x%x uncached successed\n",
...
#ifdef VENUS_DEBUG
	printf("mmaped __nndma_desram_vbase=%p, NNDMA_DESRAM_BASE=0x%x, NNDMA_DESRAM_SIZE=0x%x %s "
	   "successed\n",
	   __nndma_desram_vbase, NNDMA_DESRAM_BASE, NNDMA_DESRAM_SIZE,
	   mmap_fd == __nnafd ? "uncached accelerated" : "uncached");
#endif
	// NNDBG
	__nndbg_io_vbase = mmap(NULL, NNDBG_IO_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, __memfd,
				(off_t)NNDBG_IO_BASE);
	if (__nndbg_io_vbase == MAP_FAILED) {
	printf("Error: NNDBG IO mmap paddr=0x%x size=0x%x failed:%s\n", NNDBG_IO_BASE,
		   NNDBG_IO_SIZE, strerror(errno));
...
#endif
	// BSCALER
	__bscaler_io_vbase = mmap(NULL, BSCALER_IO_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, __memfd,
				  (off_t)BSCALER_IO_BASE);
	if (__bscaler_io_vbase == MAP_FAILED) {
	printf("Error: BSCALER IO mmap paddr=0x%x size=0x%x failed:%s\n", BSCALER_IO_BASE,
		   BSCALER_IO_SIZE, strerror(errno));
	goto err_mmap_bscaler_io;
	}

#ifdef VENUS_DEBUG
	printf("mmaped __bscaler_io_vbase=%p, BSCALER_IO_BASE=0x%x, BSCALER_IO_SIZE=0x%x uncached "
	   "successed\n",
	...
	__oram_vbase =
		mmap(NULL, oram_extend + ORAM_SIZE + oram_extend, PROT_READ | PROT_WRITE, MAP_SHARED, mmap_fd, (off_t)ORAM_BASE - oram_extend);
	...
	printf("mmaped __oram_vbase=%p, ORAM_BASE=0x%x, ORAM_SIZE=0x%x %s successed\n", __oram_vbase,
	   ORAM_BASE, ORAM_SIZE, mmap_fd == __nnafd ? "uncached accelerated" : "uncached");
#endif
	...
	buf.nmem_paddr = __ddr_pbase;
	buf.nmem_size = ddr_mem_size;
	ret = ioctl(__nnafd, IOCTL_SOC_NNA_VERSION, &buf);
	if(ret < 0)
	{
		printf("Warning : The version number is not obtained. Please upgrade the soc-nna!\n");
		oram_extend = 0;
		nmem_extend = 0;
	}else{
		if(DRIVERS_VERSION != buf.version_buf)
		{
			printf("\033[47;31merror :The soc-nna version is %08x drivers_version is  %p Don't match\033[0m\n", buf.version_buf, DRIVERS_VERSION);
			return -1;
		}else {
	...
	__ddr_vbase =
		mmap(NULL,nmem_extend + ddr_mem_size + nmem_extend, PROT_READ | PROT_WRITE, MAP_SHARED, __nnafd, (off_t)__ddr_pbase - nmem_extend);
...
//#ifdef VENUS_DEBUG
	...
	printf("mmaped __ddr_vbase=%p, __ddr_pbase=%p, ddr_mem_size=0x%x %s successed\n", __ddr_vbase,
	   __ddr_pbase, ddr_mem_size,
	   __ddr_vbase_cache_attr & NNA_CACHED
		   ? "cached"
		   : (__ddr_vbase_cache_attr & NNA_UNCACHED_ACCELERATED ? "uncached accelerated"
									: "uncached"));
#endif
	/* __nndma_fastio_vbase must be an uncached address which phyaddr should be start with
	 * 0x0******* value */
...
int __aie_get_wafer_status(){
  FILE *wafer_op = popen("devmem 0x1354022c", "r");
  if (!wafer_op)
    return 0;
  char buffer[512];
  size_t nbytes = fread(buffer, 1, 512, wafer_op);
  pclose(wafer_op);
  if(nbytes<=0)
	  return 0;
  ...
  int n=0;
  int i=2;
  for (; (buffer[i] >= '0' && buffer[i] <= '9') ||
	 (buffer[i] >= 'a' && buffer[i] <= 'z') ||
	 (buffer[i] >='A' && buffer[i] <= 'Z');++i)
    {
      if (tolower(buffer[i]) > '9'){
		  n = 16 * n + (10 + tolower(buffer[i]) - 'a');
      }
      else
	{
		n = 16 * n + (tolower(buffer[i]) - '0');
	}
    }
