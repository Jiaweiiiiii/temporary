...
int aie_mmp_set()
{
    ...
    return __aie_mmap(0x2000000, 1, desram_cache_attr, oram_cache_attr, ddr_cache_attr);
}
...
int aip_resize_test(bs_data_format_e format, uint32_t box_num)
{
    ...
    for(i = 0; i < run_cnt; i++){
        ...
        for(k = 0; k < box_num; k++) {
            dst[k].base = __aie_get_ddr_paddr((uint32_t)dst_base) + k * dst_size;
            dst[k].base1 = dst[k].base + dst_w * dst_h;
            dst[k].format = format;
            dst[k].width = dst_w;
            dst[k].height = dst_h;
...
int main(int argc, char *atgv[])
{
    ...
    int numb1 = 0;
    int numb2 = 0;
    numb2 = ingenic_aip_max_cnt(1);
    printf("ingenic_aip_p_max_cnt() = %d\n", numb2);
    numb2 = ingenic_aip_max_cnt(2);
    printf("ingenic_aip_t_max_cnt() = %d\n", numb2);
    numb2 = ingenic_aip_max_cnt(3);
    printf("ingenic_aip_f_max_cnt() = %d\n", numb2);

    printf("\n###aip_resize start, num = 1\n");
    ret = aip_resize_test(BS_DATA_NV12, 1);

    printf("\n###aip_resize start, num = 40\n");
    ret = aip_resize_test(BS_DATA_NV12, 20);
