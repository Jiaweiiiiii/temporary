...
static void *dst_base_useless_vaddr = NULL;
static void *dst_base_useless_paddr = NULL;
...
int ingenic_aip_init(void)
{
	...
	dst_base_useless_vaddr = (void *)ddr_memalign(32, 32);
	dst_base_useless_paddr = (void *)__aie_get_ddr_paddr(
			(uint32_t)dst_base_useless_vaddr);
	...
	printf("@@ Ingenic AIP LIB Version is %s @@\n", AIP_LIB_VERSION);
	pthread_mutex_unlock(&mutex_init);
...
int ingenic_aip_deinit(void)
{
	...
	if(dst_base_useless_vaddr != NULL)
	{
		ddr_free(dst_base_useless_vaddr);
		dst_base_useless_vaddr = NULL;
	}
	if(pnode)
		free(pnode);
	if(anode)
		...
		const uint32_t *coef, const uint32_t *offset)
{
	...
	uint32_t f_node_size = sizeof(struct aip_f_chainnode) * (box_num + 1);
...
#if 1
	__aie_flushcache_dir((void *)dst_base_useless_vaddr, 32, NNA_DMA_FROM_DEVICE);
	...
	node[i].src_base_y 	= src_base_y + offset_y;
	node[i].src_base_uv 	= src_base_uv + offset_uv;
	node[i].dst_base_y 	= (uint32_t)dst_base_useless_paddr;
	node[i].dst_base_uv 	= (uint32_t)dst_base_useless_paddr;
