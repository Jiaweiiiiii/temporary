...
static int manage_malloc_count = 0;
...
{
	if (manage->aip_io_vaddr != 0) {
		munmap((void *)manage->aip_io_vaddr, manage->aip_io_size);
	}
...
int aip_f_config_nodes(struct aip_manage *manage,
		struct aip_f_chainnode *chainnode, int chain_num, unsigned int cfg)
{
	uint32_t i;
	...
	if(chain_num == 0) {
		...
		for(i = 0; i < chain_num -1; i++) {
			...
			aip_f[i].cfg[1] 	= JZ_AIP_IOBASE + AIP_F_CFG;
...
int aip_p_wait()
{
	...
	if ((ret = ioctl(manage_p->fd, IOCTL_AIP_IRQ_WAIT_CMP, &status)) < 0) {
		printf("IOCTL_AIP_IRQ_WAIT_CMP failed, status = %d\n", status);
		return ret;
	}
...
int get_box_info(int type)
{
	...
	if (2 == type) {
		...
		ret = ioctl(fd, IOCTL_AIP_MALLOC, &chainbuf);
		if (ret < 0) {
			perror("Failed to obtain tnode size using IOCTL");
			return ret;
		}
	...
	if (1 == type) {
		...
		ret = ioctl(fd, IOCTL_AIP_MALLOC, &chainbuf);
		if (ret < 0) {
			perror("Failed to obtain pnode size using IOCTL");
			return ret;
		}
