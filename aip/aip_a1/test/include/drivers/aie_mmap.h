...
#define ORAM_SIZE (512 * 1024 - L2C_SIZE)
...
/**
 * Used to confirm all data to be really written to their own space, such as desram, oram, ddr and
 * so on after their corresponding instruction be executed.
 */
#define __fast_iob()                                                                               \
    __asm__ __volatile__(".set    push\n\t"                                                        \
                         ".set    noreorder\n\t"                                                   \
...
inline __attribute__((always_inline)) uint32_t __aie_get_ddr_paddr(uint32_t ddr_vaddr) {
    return (uint32_t)__ddr_pbase + (ddr_vaddr - (uint32_t)__ddr_vbase);
}
inline __attribute__((always_inline)) uint32_t __aie_get_oram_paddr(uint32_t oram_vaddr) {
    return (uint32_t)ORAM_BASE + (oram_vaddr - (uint32_t)__oram_vbase);
}
inline __attribute__((always_inline)) uint32_t __aie_get_oram_offset(uint32_t oram_vaddr) {
    return (uint32_t)L2C_SIZE + (oram_vaddr - (uint32_t)__oram_vbase);
}
inline __attribute__((always_inline)) uint32_t __aie_get_oram_vbase() {
    return (uint32_t)__oram_vbase;
}
inline __attribute__((always_inline)) uint32_t __aie_get_nndma_io_vbase() {
    return (uint32_t)__nndma_io_vbase;
}
inline __attribute__((always_inline)) uint32_t __aie_get_nndma_desram_vbase() {
    return (uint32_t)__nndma_desram_vbase;
}
inline __attribute__((always_inline)) uint32_t __aie_get_nndbg_io_vbase() {
    return (uint32_t)__nndbg_io_vbase;
}
inline __attribute__((always_inline)) uint32_t __aie_get_bscaler_io_vbase() {
